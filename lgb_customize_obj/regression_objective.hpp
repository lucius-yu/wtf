...
*/
class RegressionSmapeloss: public ObjectiveFunction {
public:
  explicit RegressionSmapeloss(const ObjectiveConfig& config) {
    eta_ = static_cast<double>(config.gaussian_eta);
  }

  explicit RegressionSmapeloss(const std::vector<std::string>&) {
  ...
  void GetGradients(const double* score, score_t* gradients,
    ...
    if (weights_ == nullptr) {
      ...
      for (data_size_t i = 0; i < num_data_; ++i) {
        const double diff = score[i] - label_[i];
        /* label_ should > 0, so diff > 0, score[i] > 0 */
        if (diff >= 0.0f) {
             gradients[i] = 2.0f * label_[i] / std::pow(score[i]+label_[i], 2.0f );
        } else {
            if (score[i] >= 0.0f) {
                gradients[i] = -2.0f * label_[i] / std::pow(score[i]+label_[i], 2.0f );
            } else { 
                gradients[i] = -1.0; /* score[i] < 0 is abnormal case, grad will be 0 */ 
            }
        }
        hessians[i] = static_cast<score_t>(Common::ApproximateHessianWithGaussian(score[i], label_[i], gradients[i], eta_));
      }
      ...
      for (data_size_t i = 0; i < num_data_; ++i) {
        const double diff = score[i] - label_[i];
        if (diff >= 0.0f) {
          gradients[i] = weights_[i] * 2.0f * label_[i] / std::pow(score[i]+label_[i], 2.0f );
        } else {
            if (score[i] >= 0.0f) {
                gradients[i] = -weights_[i] * 2.0f * label_[i] / std::pow(score[i]+label_[i], 2.0f );
            } else {
