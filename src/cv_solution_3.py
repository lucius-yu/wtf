...
'''
def solution_3(df_train, df_test, pred_len=70, skip_len=2, save_model=False, load_model=True):
    test = df_test.copy()
    ...
    for i in range(skip_len,min(pred_len,len(test_dates))+skip_len):
        ...
        if load_model:
            gbm = lgb.Booster(model_file='../models/model_'+str(i) + '_' + df_y_train.name + '.mdl')
        else:
...
if cross_validation:
    ...
    for train_start_date, train_end_date, valid_start_date, valid_end_date in valid_dates:
        ...
        s3_result = solution_3(df_train, df_test, pred_len=70, skip_len=2)
...
if submission:
    print("real submission generation")
    # real solution
    ...
    s3_result = solution_3(train, real_test, pred_len=70, skip_len=2, save_model=False, load_model=True)
    ...
    # mix with s2
    from common import mix_results
    s2_result = pd.read_csv('../submit/solution_2_raw.csv')
    s3_s2_result = mix_results(s3_result, s2_result, mix_date='2017-09-24')

    s3_s2_result.to_csv('../submit/solution_3_2_raw.csv',index=False)
    s3_s2_result[['Id','Visits']].to_csv('../submit/solution_3_2.csv', index=False)
